...
def eval_linear(root, method, N, tol, num, scen, plot = False):
    ...
    for i in range(u1.shape[0]):
        ...
        spe, pre, rec, f1, mcc, success = eval(u, Label, num, threshold = threshold)
        SPE.append(spe)
        PRE.append(pre)
        REC.append(rec)
        FS.append(f1)
...
def combine(root, combinations, method, linear=True, plot = False):
    ...
    method_dict = {'DGCCA': 'dgcca_', 'SNGCCA': '', 'PDD': 'pdd_', 'SGCCA': 'sgcca_admm_', 'SAKGCCA': 'sakgcca_', 'TSKGCCA': 'tskcca_'}
    prefix = method_dict[method]
...
if __name__ == '__main__':
    ...
    build_data(root, 'PDD', plot = True)
    build_data(root, 'TSKGCCA', plot = True)
    build_data(root, 'SAKGCCA', plot = True)