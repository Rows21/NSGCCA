...
gendata3 <- function(n,p,q,r,type){
  ...
  if(type ==1){
    ...
    Y[,1] = 1 / (X[,1] + X[,2]) - Y[,2] + matrix(rnorm(n,mean=0,sd=0.05),nrow=n,ncol=1)
    Z[,1] = 1 / (Y[,1] + Y[,2]) - Z[,2] + matrix(rnorm(n,mean=0,sd=0.05),nrow=n,ncol=1)
  } else if(type ==4){
    Y[,1] = cos(X[,1]+X[,2]) -Y[,2]
    Z[,1] = sin(X[,1]+X[,2]) -Z[,2]
  }else if(type ==5){
...
gendatan = function(i,n,p,q,r,type){
  ...
  if(i == 2){
    ...
    Y[,1] = 20 / (X[,1] + X[,2]) - Y[,2] + matrix(rnorm(n,mean=0,sd=0.05),nrow=n,ncol=1)
    Z[,1] = 20 / (X[,1] + X[,2]) - Z[,2] + matrix(rnorm(n,mean=0,sd=0.05),nrow=n,ncol=1)
  } else if(type ==4){
    Y[,1] = cos(X[,1]+X[,2]) -Y[,2]
    Z[,1] = sin(X[,1]+X[,2]) -Z[,2]
  }else if(type ==5){
...
### Linear SGCCA
```{r}
result = sgcca(Data1, connection = 1 - diag(length(Data1)), sparsity = rep(1, length(Data1)), ncomp = rep(1,
  length(Data1)), scheme = "centroid", init = "svd",
  bias = TRUE, tol = .Machine$double.eps, verbose = FALSE)
