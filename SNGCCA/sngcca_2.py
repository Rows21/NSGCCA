...
class SNGCCA2():
    ...
    def _initilize(self, view, p, rho=1):
        n, p = view.shape
        covx = np.cov(view.T)
    ...
    def fit_admm(self, views, constraint, logging=1, mode = 'compute'):
        ...
        for outer_iter in range(outer_maxiter):            
            for i, view in enumerate(self.views):
                ...
                L = 1.01 * np.sum(abs(K_tilde) * self.Z_list[i])/ (4 * n ** 2)
                Pi = self.Pi_list[i]
    ...
    def fit_admm_cp(self, views, constraint, logging=1, mode = 'compute'):
        ...
        for outer_iter in range(outer_maxiter):            
            for i, view in enumerate(self.views):
                ...
                L = 1.01 * sum(abs(K_tilde) * self.Z_list[i])/ (4 * n ** 2)
                Pi = self.Pi_list[i]
